...
public class BjczAdapter extends BaseQuickAdapter<BjczPageInfo.ContentBean,BaseViewHolder>
{
    ...
    protected void convert(BaseViewHolder helper,BjczPageInfo.ContentBean bjczInfo)
    {
        ...
        helper.setText(R.id.item_mainbjczfragment_fourline_left,new BigDecimal(bjczInfo.getAlarmValue()).stripTrailingZeros().toPlainString() + bjczInfo.getUnit().trim());
        ...
        helper.setText(R.id.item_mainbjczfragment_threeline_left,null != bjczInfo.getParentCategoryName() && !"".equals(bjczInfo.getParentCategoryName().trim()) ? bjczInfo.getParentCategoryName().trim() : "");
